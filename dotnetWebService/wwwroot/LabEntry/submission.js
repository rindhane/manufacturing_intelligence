...
function styleElem(styleEnum, fileName, elem = dropArea, displayElem = fileSelectDisplay) {
    ...
    if (styleEnum == 3) { //styling for dropped or selection of file
        ...
        for (let i = 0; i < fileName.length; i++) {
            debugger
            if (i == 0) {
                strFile = fileName[i].name;
            } else {
                strFile = strFile+",\n"+ fileName[i].name;
            }           
        }
        displayElem.innerText = strFile;
        return 2;
    }
    return 0;
}
...
function styleElemOnFileLoad(fileList) {
    ...
    if (fileList.length > 0) {
        ...
        for (var i = 0; i < fileList.length - 1; i++) {
            fileHandler = fileList[i];           
        }
        styleElem(3, fileList);
...
async function uploadFileData(fileHook, inputElem = input, checkElem = validationElem,) {
    ...
    var del = document.createElement('btnUpload');   
    ...
    for (let i = 0; i < lst.length; i++) {
       // let dat = extractBase64Data(fileList1.target.result);
        let dat = extractBase64Data(lst[i]);
    ...
    //document.getElementById("demo").innerHTML = "The file upload button now accept multiple values.";
    return true;
};
