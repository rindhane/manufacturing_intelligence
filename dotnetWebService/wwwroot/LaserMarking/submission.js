                                      ...
                                      partElem=PartSelector)
{
  ...
  if(partElem.value=="0"){
    alert ("Choose the PartCode Category for the component");
    return false;
  }
  if (LineElem.value=="0"){
    alert("select Line");
  ...
  payload = {
    ...
    senderTag:'LaserMarkingFormUpload',
    partCode: partElem.value,
  }
...
//
//function to get the partCodes configured in backend
async function getConfiguredPartCodes(){
  let options={
    method: 'POST',
    headers: {
      Accept: 'application/json',
    ...
    body: "Get PartCodes in the control plan",
    cache: 'default',
  }
  const serverMainPath = "" ; 
