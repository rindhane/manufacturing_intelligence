...
const selectStationElem = document.getElementById('LabStationDigitalBurette');
...
async function uploadScanData(inputElem = input,
    ...
    response = await postDataStream(`${serverMainPath}/LabData`, uploadData = uploadDataPDf, serialNum = inputElem.value, LabStationDigitalBurette = selectStationElem.className);
...
async function postDataStream(url, uploadData, serialNum, LabStationDigitalBurette) {
    let options = {
        method: 'POST',
        headers: {
            ...
            serialNum: serialNum,
            LabStation: LabStationDigitalBurette,
        },
        body: uploadData,
        cache: 'default',
    }
