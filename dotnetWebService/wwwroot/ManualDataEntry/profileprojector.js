...
const selectStationElem = document.getElementById('LabStationProfileProjector');
...
async function uploadScanData(inputElem = input,
   ...
   // const blob = await html2pdf().from(document.body).toPdf().outputPdf("blob");
    ...
    response = await postDataStream(`${serverMainPath}/LabData`, uploadData = uploadDataPDf, serialNum = inputElem.value, LabStationProfileProjector = selectStationElem.className);
...
async function postDataStream(url, uploadData, serialNum, LabStationProfileProjector) {
    let options = {
        method: 'POST',
        headers: {
